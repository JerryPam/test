...
public class ContrCOMPort extends Equipment{
    ...
    public String getMessage() {
        if (test_result == false & count_start == 0) { // если тест не пройден и попыток запуска оборудования еще не было
            ...
            Mes = "Ошибка при выполнении теста контроллера COM-портов \n" + "Код ошибки " + error_code;
            return Mes;
        } else if (test_result == true & count_start == 0) {
            Mes = "Тест контроллера COM-портов пройден успешно";
            return Mes;
        } else if (test_result == true & count_start == 1 & run_result == false) {
            Mes = "Ошибка запуска контроллера COM-портов";
            return Mes;
        } else {
            Mes = "Контроллера COM-портов успешно запущен";
            return Mes;
        }
    }
}